...
// Minimal jQuery double covering exactly what battle_luck_meter.js touches.
class Node {
    constructor(markup) {
...
function session() {
    ...
    const context = vm.createContext({TacticalScreenTopbarRoundInformationModule: Module, $: markup => new Node(markup),
        Screens: {MSUConnection: connection}, SQ: {call: (handle, method, message) => {
            ...
            assert.equal(method, 'battleLuckMeterLog');
            const data = {};
    ...
    const update = module.battleLuckMeterUpdate;
    let pushes = 0;
    module.battleLuckMeterUpdate = data => update.call(module, data ? {battle: 1, push: ++pushes, surface: 'battle', ...data} : data);
...
test('create binds a tooltip and stays hidden until a push', () => {
    ...
    assert.deepEqual({...root.tooltip}, {contentType: 'msu-generic', modId: 'mod_battle_luck_meter', elementId: 'Luck'});
...
test('badges default hidden without a layout gap and render exact player-coloured values when enabled', () => {
    const s = session(), view = s.module.battleLuckMeterView;
    s.module.battleLuckMeterUpdate(initial);
    assert.equal(view.readouts.style.display, 'none');
    assert.equal(view.root.hasClass('battle-luck-meter-show-percentages'), false, 'bar-only height retained');
    assert.equal(view.marker.style.left, '50%');
    s.module.battleLuckMeterUpdate({...initial, show_percentages: true, marker: 45.5, emphasis: 0.55,
    ...
    assert.equal(view.root.hasClass('battle-luck-meter-show-percentages'), true);
    assert.equal(view.oursPercent.content, '-100%');
    assert.equal(view.oursPercent.hasClass('battle-luck-meter-bad'), true);
    assert.equal(view.theirsPercent.content, '+100%');
    assert.equal(view.theirsPercent.hasClass('battle-luck-meter-bad'), true, 'enemy surplus hurts the player');
    s.module.battleLuckMeterUpdate({...initial, show_percentages: true, marker: 100, emphasis: 1,
    ...
    assert.equal(view.oursPercent.hasClass('battle-luck-meter-good'), true);
    assert.equal(view.oursPercent.hasClass('battle-luck-meter-bad'), false, 'old tone removed');
    assert.equal(view.theirsPercent.hasClass('battle-luck-meter-good'), true, 'enemy deficit helps the player');
    s.module.battleLuckMeterUpdate({...initial, enabled: false, show_percentages: true, marker: 0, ours_percent: '0%'});
    ...
    assert.equal(view.root.hasClass('battle-luck-meter-show-percentages'), false);
    s.module.battleLuckMeterUpdate(null);
...
test('Off to On to Off updates the same live view without stale values or layout', () => {
    const s = session(), view = s.module.battleLuckMeterView, id = view.id;
    s.module.battleLuckMeterUpdate({...initial, ours_percent: '-100%', ours_tone: 'bad'});
    assert.equal(view.readouts.style.display, 'none');
    s.module.battleLuckMeterUpdate({...initial, show_percentages: true, ours_percent: '+88%', ours_tone: 'good',
        theirs_percent: '+10%', theirs_tone: 'bad'});
    assert.equal(s.module.battleLuckMeterView, view);
    assert.equal(view.id, id);
    assert.equal(view.oursPercent.content, '+88%');
    assert.equal(view.oursPercent.hasClass('battle-luck-meter-good'), true);
    s.module.battleLuckMeterUpdate({...initial, ours_percent: '-29%', ours_tone: 'bad'});
    assert.equal(s.module.battleLuckMeterView, view);
    assert.equal(view.readouts.style.display, 'none');
    assert.equal(view.root.hasClass('battle-luck-meter-show-percentages'), false);
    s.module.battleLuckMeterUpdate({...initial, show_percentages: true, ours_percent: '+64%', ours_tone: 'good'});
...
test('destroy unbinds the tooltip, removes the bar, and native teardown always runs', () => {
    ...
    assert.equal(s.module.battleLuckMeterView, null);
    s.module.battleLuckMeterUpdate(initial);
    assert.deepEqual(s.errors, []);
    const t = session();
...
test('reports actual rendered fields and destroy, rejects stale or late updates', () => {
    const s = session();
    s.module.battleLuckMeterUpdate({...initial, battle: 2, push: 10, marker: 55, emphasis: 0.6, ours_percent: '+100%', ours_tone: 'good'});
    ...
    s.module.battleLuckMeterUpdate({...initial, show_percentages: true, battle: 2, push: 11, marker: 60, emphasis: 0.7,
    ...
    s.module.battleLuckMeterUpdate({...initial, battle: 1, push: 9, enabled: false});
    assert.equal(s.receipts.at(-1).status, 'stale');
    assert.equal(s.module.battleLuckMeterView.marker.style.left, '60%');
    s.module.mSQHandle = null; // The native screen disconnects first.
    s.module.destroyDIV();
    assert.equal(s.receipts.at(-1).status, 'destroyed');
    s.module.battleLuckMeterUpdate({...initial, battle: 2, push: 12});
    assert.equal(s.receipts.at(-1).status, 'missing_view');
    assert.deepEqual(s.errors, []);
    s.connection.mSQHandle = null;
    assert.doesNotThrow(() => s.module.battleLuckMeterUpdate({...initial, battle: 2, push: 13}));
